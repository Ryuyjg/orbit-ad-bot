    ...
    try:
        ...
        if is_admin(user_id):
            return {
                "max_sessions": 999,
                "can_run_ads": True,
                "ads_running": is_user_worker_running(user_id),
                "current_sessions": count_user_accounts(user_id)
            }
        
        if user_id_str in data["user_limits"]:
            limits = data["user_limits"][user_id_str].copy()
            limits["ads_running"] = is_user_worker_running(user_id)
...
# ============================================
# WORKER MANAGEMENT - FIXED FOR PARALLEL
# ============================================
def start_user_worker(user_id):
    """Start ads worker for specific user - PARALLEL SUPPORT"""
    ...
    # Check if user already has a worker running
    if is_user_worker_running(user_id):
        return True, "Ads already running for this user"
    
    try:
        # Create unique worker identifier
        worker_id = f"worker_{user_id}_{int(time.time())}"
        
        print(f"🚀 Starting parallel worker for user {user_id} (ID: {worker_id})...")
        ...
        if process.poll() is not None:
            ...
            del worker_processes[user_id]
            return False, "Worker failed to start"
        
        print(f"✅ Parallel worker started for user {user_id}, PID: {process.pid}, Worker ID: {worker_id}")
...
def stop_user_worker(user_id):
    """Stop ads worker for specific user - PARALLEL SUPPORT"""
    global worker_processes
    ...
    try:
        ...
        if user_id in worker_processes:
            worker_data = worker_processes[user_id]
            if worker_data and worker_data["process"] is not None:
...
def is_user_worker_running(user_id):
    """Check if user's worker is running - PARALLEL SUPPORT"""
    global worker_processes
...
def cleanup_workers():
    ...
    for user_id, worker_data in worker_processes.items():
        if worker_data and worker_data["process"] is not None:
            process = worker_data["process"]
            if process.poll() is not None:
...
async def start_handler(event):
    ...
    running_workers = get_running_workers_count()
    ...
    await event.reply(
        ...
        f"⚡ Ads Status: {status}\n"
        f"👥 Parallel Workers: {running_workers} running\n"
...
async def account_tools_callback(event):
    ...
    if can_add:
        buttons.append([
            ...
            Button.inline("🚀 BULK ADD SESSIONS", b"bulk_sessions")
        ])
    else:
    ...
    msg += f"Ads Running: {'🟢 Yes' if is_user_worker_running(user_id) else '🔴 No'}\n\n"
    msg += f"**New Feature:** 📦 Bulk Session Import\n"
    msg += f"Add multiple sessions at once with API_ID, API_HASH, SESSION_STRING format!"
...
# BULK SESSIONS (FIXED)
# ============================================
@bot.on(events.CallbackQuery(data=b"bulk_sessions"))
async def bulk_sessions_callback(event):
    user_id = event.sender_id
    ...
    instructions = (
        ...
        "**Format (one account per block, separate with '---'):**\n"
        "`API_ID`\n"
        "`API_HASH`\n"
        "`SESSION_STRING`\n"
        "`---` (separator)\n\n"
        ...
        "1234567\n"
        "abc123def456ghi789\n"
        "1BVtsOIYBu0-mGm2xrwtDmQ... (session string)\n"
        "---\n"
        "7654321\n"
        "xyz789abc456def123\n"
        "1BVtsOK8Bu8RfyNY61B3yk... (session string)\n"
        "---\n\n"
        "Send multiple sessions at once!"
    )
    
    await event.respond(instructions)
    session_waiting[user_id] = {"step": "bulk_import"}
...
async def user_manager_callback(event):
    ...
    await event.edit(
        f"👥 USER MANAGER\n"
        f"Parallel Workers Running: {running_workers}\n\n"
        f"Choose an action:",
        buttons=buttons
    )
...
async def show_users_callback(event):
    ...
    for user in users:
        ...
        ads_status = "🟢" if user["ads_running"] else "🔴"
        button_text = f"👤 {username} ({ads_status} {user['current_sessions']}/{user['max_sessions']})"
...
async def status_callback(event):
    ...
    msg += f"Ads Permission: {'✅ Yes' if limits['can_run_ads'] else '❌ No'}\n"
    msg += f"Parallel Workers Running: {running_workers}"
...
async def message_handler(event):
    ...
    # Handle session count (old single session method)
    ...
    # Handle session details (old single session method)
    ...
    # Handle OTP input (old single session method)
    ...
    # Handle 2FA input (old single session method)
    ...
    # Handle bulk session input (NEW FIXED METHOD)
    if user_id in session_waiting and session_waiting[user_id].get("step") == "bulk_import":
        try:
            ...
            for line in lines:
                ...
                if line == '---':
                    if current_account:
                        if all(key in current_account for key in ['api_id', 'api_hash', 'session_string']):
                ...
                elif 'session_string' not in current_account:
                    current_account['session_string'] = line
                    accounts.append(current_account)
                    current_account = {}
            
            # Add last account if exists
            if current_account and all(key in current_account for key in ['api_id', 'api_hash', 'session_string']):
                accounts.append(current_account)
            ...
            if len(accounts) > remaining:
                await event.respond(f"❌ You can only add {remaining} more sessions! You tried to add {len(accounts)}.")
                return
            
            # Process accounts
            added_count = 0
            failed_count = 0
            progress_msg = await event.respond(f"📦 Processing {len(accounts)} sessions...")
                ...
                try:
                    ...
                    if success:
                        ...
                        if i % 5 == 0 or i == len(accounts):
                            await progress_msg.edit(f"📦 Processing... {i}/{len(accounts)} added")
                    else:
                        failed_count += 1
                        print(f"Failed to add account {i}: {message}")
            ...
            await progress_msg.edit(
                ...
                f"• 📱 Total now: {count_user_accounts(user_id)}/{limits['max_sessions']}"
            )
            
            del session_waiting[user_id]
            
        except Exception as e:
            await event.respond(f"❌ Error processing bulk import: {str(e)}")
        return
